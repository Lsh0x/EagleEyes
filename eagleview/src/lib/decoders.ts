...
function decodeL4(u: Uint8Array, off: number, proto: number, isv6 = false): { l4: Decoded['l4']; summary: string; tag: string; meta?: Decoded['meta'] } {
  if (proto === 6 && u.length >= off + 20) { // TCP
    ...
    if (srcPort === 25 || dstPort === 25 || srcPort === 587 || dstPort === 587) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'SMTP', tag: 'SMTP', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 465 || dstPort === 465) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'SMTPS', tag: 'SMTPS', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 110 || dstPort === 110) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'POP3', tag: 'POP3', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 143 || dstPort === 143) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'IMAP', tag: 'IMAP', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 993 || dstPort === 993) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'IMAPS', tag: 'IMAPS', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 389 || dstPort === 389) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'LDAP', tag: 'LDAP', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 636 || dstPort === 636) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'LDAPS', tag: 'LDAPS', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 139 || dstPort === 139 || srcPort === 445 || dstPort === 445) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'SMB', tag: 'SMB', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 3389 || dstPort === 3389) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'RDP', tag: 'RDP', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 554 || dstPort === 554) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'RTSP', tag: 'RTSP', meta: { tcp: { flags: flagsByte } } }
    if (srcPort === 5060 || dstPort === 5060) return { l4: { proto: 'TCP', srcPort, dstPort, tcpFlags: flags }, summary: 'SIP', tag: 'SIP', meta: { tcp: { flags: flagsByte } } }
  ...
  if (proto === 17 && u.length >= off + 8) { // UDP
    ...
    if (srcPort === 161 || dstPort === 161 || srcPort === 162 || dstPort === 162) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'SNMP', tag: 'SNMP' }
    if (srcPort === 514 || dstPort === 514) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'Syslog', tag: 'SYSLOG' }
    if (srcPort === 69 || dstPort === 69) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'TFTP', tag: 'TFTP' }
    if (srcPort === 137 || dstPort === 137 || srcPort === 138 || dstPort === 138) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'NetBIOS', tag: 'NETBIOS' }
    if (srcPort === 546 || dstPort === 546 || srcPort === 547 || dstPort === 547) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'DHCPv6', tag: 'DHCPv6' }
    if (srcPort === 520 || dstPort === 520) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'RIP', tag: 'RIP' }
    if (srcPort === 5060 || dstPort === 5060) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'SIP', tag: 'SIP' }
    // RTP/RTCP heuristics
    const first = u[off]
    const second = u[off + 1]
    const v2 = (first & 0xc0) === 0x80
    if (v2) {
      if (second >= 200 && second <= 204) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'RTCP', tag: 'RTCP' }
      if (u.length >= off + 12) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'RTP', tag: 'RTP' }
    }
    if (srcPort === 443 || dstPort === 443) return { l4: { proto: 'UDP', srcPort, dstPort }, summary: 'QUIC', tag: 'QUIC' }
